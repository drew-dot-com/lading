...
test('readFirst: the first gateway that answers 2xx wins', async () => {
  ...
  const r = await readFirst(arweaveReadUrls(TX, ['permagate.io', 'arweave.net', 'ardrive.net']), f);
...
test('readFirst: every gateway down reports the last status and all attempts', async () => {
  const r = await readFirst(arweaveReadUrls(TX, ['permagate.io', 'arweave.net']), answer(502));
